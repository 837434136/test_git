package com.yisiinian.httputils.utils;
...
public class BackAES {
	...
	public static String encrypt(String sSrc, String sKey) throws Exception {
		...
		Cipher cipher = Cipher.getInstance(selectMod(0));
		byte[] raw = sKey.getBytes();
		SecretKeySpec skeySpec = new SecretKeySpec(raw, WAYS);

		IvParameterSpec iv = new IvParameterSpec(ivParameter.getBytes());// 使用CBC模式，需要一个向量iv，可增加加密算法的强庿
		...
		return URLEncoder.encode(new String(Base64.encode(encrypted)), "UTF-8");// 此处使用BASE64做转码㿿
	}
...
//		}
	...
	public static byte[] newencrypt(String content, String password) {
		try {
			...
			Cipher cipher = Cipher.getInstance("AES");// 创建AES加密编码噿
	...
	/*
	 * @param content 待解密内宿,格式为byte数组
	 * 
	 * @param password AES解密使用的密钿
	 * 
	 * @return
	 */
